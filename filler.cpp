...
{
    borderColorPicker bcp(borderColor, img, tolerance, *img.getPixel(x,y));
                       ...
                       double tolerance, int frameFreq)
{
    ...
    visited.push_back(pair<int,int>(x,y));
    int temp;
    ...
    while(!os.isEmpty()){
        ...
        for (int i = x+1; x >= x-1; x--){
            if (!contains(visited,pair<int,int>(i,y-1))){
                os.add(pair<int,int>(i,y-1));
                fillColor(i,y-1);
                visited.push_back(pair<int,int>(i,y-1));
                temp++;
                if (temp == frameFreq){
                    anime.addFrame(img);
                    temp = 0;
        ...
        if (!contains(visited,pair<int,int>(x-1,y))){
            os.add(pair<int,int>(x-1,y));
            fillColor(x-1,y);
            visited.push_back(pair<int,int>(x-1,y));
                temp++;
                if (temp == frameFreq){
                    anime.addFrame(img);
                    temp = 0;
        ...
        for (int j = x-1; j<= x+1; j++){
            if (!contains(visited,pair<int,int>(j,y+1))){
                os.add(pair<int,int>(j,y+1));
                fillColor(j,y+1);
                visited.push_back(pair<int,int>(j,y+1));
                temp++;
                if (temp == frameFreq){
                    anime.addFrame(img);
                    temp = 0;
    ...
    bool filler::contains(vector<pair<int,int>> visited, pair<int,int> curr){
